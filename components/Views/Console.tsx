...
const Console: React.FC<ConsoleProps> = ({ serverId }) => {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-120px)] rounded-xl border border-border bg-[#09090b] overflow-hidden shadow-2xl animate-fade-in ring-1 ring-border/50 relative">
            {/* Header: Status & Controls */}
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between px-4 py-3 border-b border-border bg-muted/10 gap-4">
            ...
            <div 
                ...
                style={{ 
                    fontSize: `${user.preferences.terminal.fontSize}px`,
            ...
            {/* Command Input */}
            <div className="bg-muted/10 p-3 border-t border-border z-10">
                <form onSubmit={handleSend} className="flex gap-2 items-center bg-[#09090b] border border-border rounded-lg px-3 py-2.5 focus-within:ring-1 focus-within:ring-primary/50 focus-within:border-primary/50 transition-all shadow-inner">
                    ...
                    <input
                        ...
                        disabled={status === ServerStatus.OFFLINE}
                        className="flex-1 bg-transparent border-none text-sm font-mono text-foreground focus:outline-none placeholder:text-muted-foreground/30 disabled:cursor-not-allowed"
                    />
                    <button 
                        type="submit"
