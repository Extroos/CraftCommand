...
class ProcessManager extends EventEmitter {
    ...
    public cleanupServer(id: string) {
        ...
        // 3. Clear Listeners (Detaching from the runner explicitly)
        const listeners = this.runnerListeners.get(id);
        const runner = this.activeRunners.get(id); // Re-check if runner is still in memory somehow
        if (listeners && runner) {
