...
class ProcessManager extends EventEmitter {
    ...
    async findAndKillGhostProcess(port: number, workingDirectory: string) {
        try {
            console.log(`[ProcessManager] Scanning for ghost processes on port ${port}...`);
            const connections = await si.networkConnections();
            const listener = connections.find(c => 
            ...
            if (listener && listener.pid) {
                const procs = await si.processes();
                const target = procs.list.find(p => p.pid === listener.pid);
                
                if (target && (target.name.toLowerCase().includes('java') || target.command.toLowerCase().includes('java'))) {
                    console.warn(`[ProcessManager] Found ghost Java process (PID: ${listener.pid}) on port ${port}. Terminating...`);
                    try {
            ...
            console.error('[ProcessManager] Failed to cleanup ghost process:', e);
        }
        return false;
    }
