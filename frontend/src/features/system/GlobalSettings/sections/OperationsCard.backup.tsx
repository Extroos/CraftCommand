...
import { Settings2, Shield, Database, Activity, AlertTriangle, Layers } from 'lucide-react';
            ...
            <div className="space-y-3">
                ...
                <div className="pt-4 mt-4 border-t border-border/40">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h3 className="text-sm font-bold tracking-tight text-foreground">Execution Architecture</h3>
                            <p className="text-[10px] font-medium text-muted-foreground">Define how Minecraft processes are spawned and managed.</p>
                        </div>
                    </div>

                    <div className="space-y-4">
                        <div className="flex flex-col gap-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Default Engine</label>
                            <select
                            ...
                            >
                                <option value="native">Native (Direct Child)</option>
                                <option value="remote">Node Agent (Daemon - Recommended)</option>
                                <option value="docker" disabled={!settings.app.dockerEnabled}>Docker Container (Isolated)</option>
                            </select>
                        </div>

                        <div className="space-y-3 px-1">
                            <p className="text-[10px] text-muted-foreground leading-relaxed font-medium">
                                The <strong>Node Agent</strong> architecture spawns servers as detached background daemons, ensuring they stay online if the Panel is closed. 
                                <strong> Docker</strong> provides full resource isolation and eliminates Java version conflicts by containerizing each server.
                            </p>
                            
                            {!settings.app.dockerEnabled && settings.app.defaultExecutionEngine === 'docker' && (
                                <div className="flex items-center gap-2 text-rose-500 font-bold text-[9px] bg-rose-500/5 border border-rose-500/10 p-2 rounded">
                                    <AlertTriangle size={12} />
                                    <span>Warning: Docker is currently selected as default but is disabled in "Operational Mode" above.</span>
                                </div>
                            )}

                            <div className="flex items-start gap-2 text-amber-500/90 font-bold text-[10px] bg-amber-500/5 border border-amber-500/10 p-2.5 rounded">
                                <AlertTriangle size={14} className="shrink-0 mt-0.5" /> 
                                <span>Note: You must restart the Panel (localhost:3000) terminal for architectural changes to initialize.</span>
                            </div>
                        </div>
                    </div>
                </div>
